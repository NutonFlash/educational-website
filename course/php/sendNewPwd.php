...
if (isset($data['email'])) {
    try {
        ...
        if ($result->rowCount() === 0) {
            ...
            $message = str_replace('$newPwd', $password, strval($message));
            ...
            $db->update(['password' => $password_hash])->table('users')->where(new Grouping('OR', new Conditional('email', '=', $data['email']), new Conditional('login', '=', $data['email'])))->execute();
            $responseCode = 0;
        }
